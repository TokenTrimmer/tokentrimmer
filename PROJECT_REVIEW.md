...
**◻ Remaining — public, doable:** _none in this review's scope — all shipped._ Open backlog work is now CLOUD-repo or BLOCKED on external accounts/migrations (plus optional follow-ups noted on shipped items: streaming failover, plan-core pricing externalization, the `agent-no-termination` AST migration, and vendoring real pinned OSS into `corpora/`).
