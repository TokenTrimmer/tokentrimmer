...
cost cap, runaway-repeat detection). Every run is signable as a receipt
(`POST /v1/agent-runs/{run_id}/receipt/sign` in cloud) + verifiable offline
...
| `max_cost_usd` | `Option<f64>` | Hard ceiling on the run's accumulated served cost (USD). When set, the loop terminates as `Incomplete` with `stop_reason = budget_exhausted` before starting a turn that would breach it. `None` ⇒ no cost cap. |
...
`Message::Tool`. The run rolls into `saved_usd` + is signable as a receipt.
...
- The cloud receipt endpoint — `POST /v1/agent-runs/{run_id}/receipt/sign` — mints a signed receipt for a completed run.
