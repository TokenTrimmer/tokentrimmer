...
1. **Off-by-default / output unchanged from today.** No panel header ⇒ the transcoded `/v1/messages` and `/v1/responses` bodies + streams are unchanged from current behavior: no `tokentrimmer` key grafted, no extra SSE frames. (The transcoders already re-serialize via shape conversion, so this is "identical to today's transcoded output," not "byte-identical to the chat body" — the only delta Phase 6 introduces is the conditional `tokentrimmer.panel` key, present strictly when the pluck is `Some`.)
