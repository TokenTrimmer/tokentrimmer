...
### The handler refactor (the main risk)
The chat handler's completion logic must become an in-process callable the loop invokes per turn, **without duplicating it or regressing `/v1/chat/completions`**. This is the money + circuit-breaker hot path (the ARCH-1 stuck-breaker bug lived here), so the refactor MUST be behavior-preserving: the existing `handler` becomes a thin wrapper that builds the request context and calls the extracted core exactly as before; the loop calls the same core. The extracted core's contract: given a resolved org/auth context + a `ChatCompletionRequest`, perform one routed+metered completion and return the assistant message + usage (or an error). The plan must keep all existing chat tests green as the behavior-preservation gate.
