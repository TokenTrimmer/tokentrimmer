...
- **D5 — Failure posture mirrors the existing streaming path; no new fallback.**
  ...
  - Hang safety: the arbiter stream is under the outer route `TimeoutLayer` (60s hardcap). A stalled stream is aborted by the timeout → the body drops → `DropGuard` fires → the row is written. Panel streams respect the same timeout contract as single-model streams.
