...
| `X-TokenTrimmer-Route` | Force a specific named route, ignoring its conditions (unknown name or known-incompatible target → `400`; policy storage unavailable → `503`; chat completions only). A force does not bypass a pause. | Honored | `cheap-for-short` |
...
| `X-TokenTrimmer-Route-Matched` | the selected route's name (forced or condition-matched); does not imply its model rewrite executed. Paused or capability-suppressed chat routes retain privacy effects on the caller model. | `cheap-for-short` |
