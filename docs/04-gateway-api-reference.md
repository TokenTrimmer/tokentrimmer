...
| `X-TokenTrimmer-Captured` | present **only** when the trace's request/response body was handed to the per-org **opt-in encrypted body-capture** sink (the hosted feature is armed AND your org enabled capture in its settings). Always `true` when present; **absent** on every default-path response (capture off by default). See [§19 — does Gateway log my prompts?](#19-faq) and the [hosted privacy spec](tokentrimmer-architecture-spec-v1.md#161-customer-data). | `true` |
...
- stamps `X-TokenTrimmer-Captured: true` on responses whose body was handed to the capture sink, so a caller can always tell whether a given trace was captured. The header is **absent** on every default-path response.
