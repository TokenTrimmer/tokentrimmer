...
- [ ] [P2] [gw-gdpr-delete-export] rust-crate-builder: Complete the self-serve "delete my org / export my data" flow across public and Cloud stores, subscription/identity teardown, and documented retention windows. **PARTIAL:** the public gateway now exposes a signed, idempotent org-cache purge primitive with a write fence, and Agent transcripts have tenant-scoped export/erasure; the Cloud-owned coordinator, full retained-data export, and remaining store cleanup are still open. Pairs with `rv-l2-org-cache-optout`. (audit: backlog-completeness) (est: $1.50)
