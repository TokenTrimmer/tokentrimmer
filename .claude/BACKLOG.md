...
- Any remaining G-series items in this file and all other open P1–P3 backlog entries that are not P0-tagged.
...
- [x] [P3] [gw-traceparent-ingest] rust-crate-builder: Honor the inbound W3C `traceparent` **request** header (doc'd "Planned (not yet honored)") — ingest it via the OTel `TraceContextPropagator` in the request trace middleware so the gateway request span continues the caller's distributed trace (parent = inbound span, same trace_id); `tracestate` preserved; absent/malformed → fresh root. Doc row flipped to Honored (standardised the header name from `X-TokenTrimmer-Trace-Parent` to the canonical `traceparent`). (est: $0.80) — DONE: `tt_telemetry::propagation` + `crates/core/src/middleware/trace.rs`.
...
- [ ] [P2] [gw-gdpr-delete-export] rust-crate-builder: Complete the self-serve "delete my org / export my data" flow across public and Cloud stores, subscription/identity teardown, and documented retention windows. **PARTIAL:** the public gateway now exposes a signed, idempotent org-cache purge primitive with a write fence, Agent transcripts have tenant-scoped export/erasure, and the separate per-org L2 opt-out is complete; the Cloud-owned coordinator, full retained-data export, and remaining store cleanup are still open. (audit: backlog-completeness) (est: $1.50)
...
- [ ] [P2] [l2-dedup-cloud-endpoint] cloud (private repo): Surface `tt_cache::L2Cache::analyze_dedup` via `POST /orgs/{org_id}/cache/analyze-dedup` (returns `DedupReport`: cluster_count / entries_deduped ratio / top clusters by hit_count). The public-side trait capability is complete, read-only, and unit-tested (cache/src/l2.rs); only the cloud HTTP surface remains. [DEFERRED — cloud private repo, out of scope for public worktree] (est: $0.40)
- [ ] [P2] [rv-batch-api] rust-crate-builder: Complete automatic, provider-neutral Batch dispatch keyed off a route action / `tag=background`, including Anthropic Message Batches and realized-savings reconciliation. **PARTIAL:** the manual OpenAI Batch Lane now implements authenticated file upload/download plus durable create/list/get/cancel through the gateway, Rust client, and CLI; `then.batch` remains advisory and does not enqueue synchronous traffic. The remaining admission→submit→poll→retrieve→reconcile behavior still needs a spec. (§2.16) (est: $1.50)
...
- **~~SKIP: general-purpose workflow canvas builder~~ → REVERSED 2026-06-27 (owner-approved).** Now building the **Agent Workflow Builder** (TT-hosted durable engine + visual canvas) — see `docs/superpowers/specs/2026-06-27-agent-workflow-builder-design.md` + `COMPREHENSIVE_REVIEW_2026-06-27.md` §3 (W0–W5). The original wedge-dilution + secrets-vault concerns are mitigated by the bounded framing: cost-control + verifiable per-workflow receipt IS the wedge (not a generic canvas); BYO-secrets only (no OAuth custody); network-less sandboxed Transform nodes. *Original rationale (kept for the record): dilutes the "swap a base URL, save money, prove it" wedge into n8n/Zapier/Dify territory; adds a third-party-secrets-vault liability.*
- **~~SKIP: internet connectors / third-party credential custody~~ → PARTIALLY REVERSED 2026-06-27.** The Workflow Builder ships **egress-guarded HTTP/API nodes with BYO-secrets** (per-org XChaCha20 vault or ephemeral run inputs; `url_guard`/`dns_guard`; domain allowlist) — see the spec §3/§11. **Still SKIPPED: custodied third-party OAuth / a Zapier-class connector marketplace** (the original Zapier-class secrets-vault + SSRF liability stands for *that*). Revisit-trigger guard: if a slice needs custodied OAuth connectors, stop and re-decide. *Original substitute (still valid): be the cost-aware node inside builders users already run (n8n node, base-URL-swap docs) + MCP write-tools (#149).*
