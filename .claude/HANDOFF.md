...
_Written at 2026-09-15T01:27:07Z by session `20260915-60609` on branch `main` (@ ecafa22)._

## Status: C05 D4 completed: the last production micros copy (run_receipt_savings) now delegates to tt-shared SignedMoneyMicros; all three Cloud receipt/showback conversions share one rule. Confirmed the existing --features pdf cross-surface population-parity test (digest/PDF/run-receipt/monthly-report) still passes after the D2 NUMERIC money migration. Merged Cloud #653. Verified: cloud tt-api run_receipt 10/0 and the parity test 1/0. No deployment or launch approval.

Active task: `c05-d4-complete`
...
C05 remaining: D1 per-request computation adoption and a golden UI/API/export/receipt cohort against a running candidate. Keep root totals at 3 local complete / 42 partial / 11 external / 3 open.
