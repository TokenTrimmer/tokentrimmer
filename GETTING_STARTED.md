...
The authoritative mode, target, feature, and credential contract is generated by `tt proxy --help` from the same `ModeContract` used by startup validation and every forwarded request. Run it instead of relying on a copied mode table.

Session log is JSONL at `~/.tokentrimmer/sessions/YYYY-MM-DD.jsonl`; on Ctrl-C the proxy prints a session summary.

`tt proxy --help` also identifies each mode's required TokenTrimmer key, provider credential, or loopback Gateway target.
