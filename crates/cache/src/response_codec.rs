...
/// `"neg:"` (→ `"neg:{org_id}:{request_hash}"`). This parses the leading
/// `org_id` from either shape so the L1 codec can derive the per-org key from
/// the key alone (the [`crate::L1Cache`] trait carries no org argument).
///
