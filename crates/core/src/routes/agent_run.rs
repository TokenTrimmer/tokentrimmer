//! Agent-run HTTP transport and gateway adapters.
//!
//! The resumable state machine and stop/budget policy live in `loop_core`;
//! Redis/Postgres serialization and org-scoped lookup live in `persistence`.
//! This facade owns Axum request/response handling plus production provider and
//! summarizer adapters. Tests can inject `TurnCompleter` into the execution
//! boundary without constructing HTTP or storage state.
...
use tt_shared::{
    context::{
        BudgetDispatchState, ProviderCredentials, RunBudgetSnapshot, RunBudgetState, SecretString,
    },
...
pub(crate) use persistence::{
    durable_record_to_run, fetch_run, run_key, store_run, StoredRouting, StoredRun, SummarizeConfig,
};
pub use persistence::{list_runs, DurableRunView, ListRunsResponse};

#[cfg(test)]
use loop_core::is_mechanical_continuation;
use loop_core::{
    eligible_tool_ordinals, latest_user_text, run_loop_core_with_output_cap, sample_key,
    token_true_ok,
};
pub use loop_core::{run_loop, Run, RunEvent, RunStatus, RunUsage, TurnCompleter};
pub(crate) use loop_core::{run_loop_core, LoopOutcome, TranscriptSummarizer, DEFAULT_MAX_TURNS};
...
struct GatewayCompleter<'a> {
    ...
    /// Clone-shared dispatch ordinals and optional run cash reservation ledger.
    budget_dispatch: BudgetDispatchState,
...
impl TurnCompleter for GatewayCompleter<'_> {
    async fn complete(
        ...
        let mut ctx = RequestContext {
            budget_dispatch: self.budget_dispatch.clone(),
        ...
        // caller's run cash ceiling before primary, retry, failover, panel,
        // shadow, summarizer, judge, or embedding work and settles each attempt
        // from provider usage (or its conservative admitted estimate). The
        // durable org/key reservation remains a second, cross-replica guard.
        // Cache stays disabled so an agent turn cannot bypass either ledger.
...
/// Variant used by workflow nodes with an additional single-dispatch cap.
/// Workflow admission strips work that could fan out, then re-prices the final
/// routed request; the shared provider decorator remains the settlement choke
/// point.
#[allow(clippy::too_many_arguments)]
async fn drive_run_loop_with_output_cap(
    ...
    let base_ctx = base_request_context(&identity, budget_dispatch.clone());
...
pub(crate) async fn drive_workflow_node(
    ...
    let budget_dispatch = budget_dispatch_for_run(&identity, max_cost_usd, 0.0);
    drive_run_loop_with_output_cap(
        state,
        identity,
        budget_dispatch,
...
async fn persist_paused(
    ...
    summarize_cfg: Option<SummarizeConfig>,
    run_budget_snapshot: Option<RunBudgetSnapshot>,
) -> ApiResult<Run> {
    match state.l1.as_ref() {
        Some(l1) => {
            let stored = StoredRun {
                ...
                summarized_upto,
                summarizer_tax_usd,
                run_budget_settled_micros: run_budget_snapshot
                    .as_ref()
                    .map(|snapshot| snapshot.settled_micros),
                run_budget_components: run_budget_snapshot
                    .map(|snapshot| snapshot.components)
                    .unwrap_or_default(),
...
pub async fn create_run(
    ...
    if max_cost_usd.is_some_and(|cap| RunBudgetState::from_usd(cap, 0.0).is_none()) {
        return Err(ApiError::InvalidRequest(
            "max_cost_usd must be a finite non-negative micro-USD-representable value".into(),
        ));
    }
    ...
    identity.run_id = id;
    let budget_dispatch = budget_dispatch_for_run(&identity, max_cost_usd, 0.0);
    let run_budget = budget_dispatch.run_budget().cloned();
        ...
        tokio::spawn(async move {
            ...
            match outcome {
                LoopOutcome::Terminal(run) => {
                    ...
                    let settled_cost_usd = run_budget
                        .as_ref()
                        .map(|budget| budget.snapshot().settled_micros as f64 / 1_000_000.0)
                        .unwrap_or(run.usage.cost_usd);
                ...
                LoopOutcome::Paused {
                    ...
                    match persist_paused(
                        ...
                        summarizer_tax_usd,
                        summarize_cfg,
                        run_budget.as_ref().map(RunBudgetState::snapshot),
                    )
                    .await
                    {
                        Ok(run) => {
...
pub async fn submit_tool_outputs(
    ...
    let budget_dispatch = budget_dispatch_for_resume(&identity, &stored);
    let run_budget = budget_dispatch.run_budget().cloned();
    ...
    match outcome {
        ...
        LoopOutcome::Terminal(mut run) => {
            ...
            if let Some(snapshot) = run_budget.as_ref().map(RunBudgetState::snapshot) {
                stored.run_budget_settled_micros = Some(snapshot.settled_micros);
                stored.run_budget_components = snapshot.components;
            }
            store_run(l1.cache.as_ref(), &stored).await?;
            // W0b Task 5: finalize agent_runs row (best-effort; pool absence ⇒ skip).
...
impl GatewayTranscriptSummarizer<'_> {
    ...
    fn maybe_spawn_summary_judge(
        ...
        // A detached provider call could settle after the run's terminal or
        // paused evidence is emitted. Agent runs therefore do not launch the
        // off-path summary judge; synchronous summarizer calls remain metered.
        if self.base_ctx.run_id.is_some() {
            return;
        }
...
async fn resolve_summarize_config(
    ...
    let ctx = base_request_context(
        identity,
        crate::budget_reservation::dispatch_state_for_idempotency(
            identity.org_id,
            identity.api_key_id,
...
/// Build one clone-shared dispatch state for every primary and auxiliary call
/// in a run segment. Invalid persisted/internal caps become a zero-cap ledger:
/// fail closed rather than silently dropping reservation enforcement.
fn budget_dispatch_for_run(
    identity: &RunIdentity,
    max_cost_usd: Option<f64>,
    already_settled_usd: f64,
) -> BudgetDispatchState {
    let run_budget = max_cost_usd.map(|cap| {
        RunBudgetState::from_usd(cap, already_settled_usd)
            .or_else(|| RunBudgetState::from_usd(0.0, 0.0))
            .expect("zero run budget is valid")
    });
    crate::budget_reservation::dispatch_state_for_idempotency(
        identity.org_id,
        identity.api_key_id,
...
fn budget_dispatch_for_resume(identity: &RunIdentity, stored: &StoredRun) -> BudgetDispatchState {
    let run_budget = stored.max_cost_usd.map(|cap| {
        let restored = match stored.run_budget_settled_micros {
            Some(settled_micros) => RunBudgetState::from_persisted(
                cap,
                settled_micros,
                stored.run_budget_components.clone(),
            ),
            None => RunBudgetState::from_usd(
                cap,
                stored.usage.cost_usd + stored.summarizer_tax_usd.unwrap_or(0.0),
            ),
        };
        restored
            .or_else(|| RunBudgetState::from_usd(0.0, 0.0))
...
mod tests {
    ...
    #[test]
    fn run_dispatch_state_shares_remaining_cash_with_auxiliary_contexts() {
        let mut identity =
            RunIdentity::from_request(None, "", &HeaderMap::new()).expect("identity");
        identity.run_id = Uuid::from_u128(9);
        let dispatch = budget_dispatch_for_run(&identity, Some(1.0), 0.25);
        let base_ctx = base_request_context(&identity, dispatch.clone());
        let budget = dispatch.run_budget().expect("capped run ledger");

        let reservation = budget
            .reserve([7; 32], "openai", "chat", "gpt-test", Some(0.5))
            .unwrap();
        budget.settle(
            reservation,
            Some(0.125),
            tt_shared::context::RunBudgetSettlementBasis::ProviderUsage,
        );

        assert_eq!(base_ctx.run_id, Some(identity.run_id));
        assert_eq!(
            base_ctx
                .budget_dispatch
                .run_budget()
    ...
    async fn stored_run_roundtrips_through_cache() {
        ...
        let run = StoredRun {
            ...
            summarized_upto: 0,
            summarizer_tax_usd: None,
            run_budget_settled_micros: Some(125_000),
            run_budget_components: vec![],
            summarize: None,
            max_cost_usd: Some(1.0),
            stop_reason: None,
        };
        ...
        assert_eq!(got.run_budget_settled_micros, Some(125_000));
        let identity = RunIdentity::from_request(None, "", &HeaderMap::new()).unwrap();
        assert_eq!(
            budget_dispatch_for_resume(&identity, &got)
                .run_budget()
                .unwrap()
                .snapshot()
                .remaining_micros(),
    ...
    async fn paused_with_l1_persists_requires_action() {
        ...
        let stored = StoredRun {
            ...
            summarized_upto: 0,
            summarizer_tax_usd: None,
            run_budget_settled_micros: None,
            run_budget_components: vec![],
            summarize: None,
            max_cost_usd: None,
            stop_reason: None,
    ...
    fn stored_run_deserializes_without_new_fields() {
        // A run persisted before this deploy has no summary or run-budget
        // fields; serde defaults must keep it resumable.
        let json = r#"{
        ...
        assert_eq!(sr.run_budget_settled_micros, None);
        assert!(sr.run_budget_components.is_empty());
    ...
    struct BudgetDeniedStub;

    #[async_trait]
    impl TurnCompleter for BudgetDeniedStub {
        async fn complete(
            &self,
            _req: ChatCompletionRequest,
            _is_mechanical: bool,
    ...
    #[tokio::test]
    async fn provider_reservation_denial_is_budget_exhaustion_not_run_failure() {
        let run = run_loop(
            &BudgetDeniedStub,
            uuid::Uuid::nil(),
            "m".into(),
        ...
        assert_eq!(run.stop_reason, Some(StopReason::BudgetExhausted));
    }
