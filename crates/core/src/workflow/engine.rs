//! Workflow engine facade and stable result contracts.
//!
//! Mutable DAG execution and recursive scheduling live in `engine::execution`;
//! pure graph/template evaluation lives in `engine::evaluation`; persistence
//! remains behind the workflow store and node-run-store modules; SSE wire
//! events remain in `workflow::events`.
//!
...
use evaluation::{
    build_union_adj, capture_input, eval_cond, propagate_edges, substitute, topo_sort,
};
use execution::run_workflow_boxed;
