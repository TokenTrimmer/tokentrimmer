...
pub enum NodeKind {
    ...
    /// D6 — distill a document's text layer to text downstream nodes can use.
    ///
    /// Reuses the gateway's Document Lane seam, so workflow and routed-request
    /// extraction agree. The cache identity covers organization, decoded source
    /// bytes (BLAKE3), normalized media type, optional caller key, immutable
    /// sidecar revision, and TokenTrimmer policy revision. A hit returns cached
    /// text without a sidecar call; missing revision provenance bypasses reuse.
    /// A miss calls the sidecar and stores a successful result. A sidecar error
    /// emits an error `NodeOutput`; raw bytes are never mislabeled as text.
    ///
    /// `source` may be inline base64 or an HTTPS URL fetched through the same
    /// no-redirect, DNS-pinned, closed-media-set, byte-capped transport as the
    /// workflow HTTP node. `cache_key`, when present, is a template such as
    /// `"{{trigger.input_id}}"` composed into the semantic cache identity.
    Document {
