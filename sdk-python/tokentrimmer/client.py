...
- ``gateway.models()`` and ``gateway.capabilities()`` provide bounded,
  runtime-validated responder metadata.
...
class TokenTrimmer(OpenAI):
    ...
    def gateway(self) -> GatewayMetadata:
        """Bounded model-catalog and authenticated capability metadata reads."""
